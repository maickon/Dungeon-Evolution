...
class Ombreiras{
	...
	function escolher_ombreiras(){
		$file = new Arquivo();
		$ombreiras = array();
		$file->open_file('arquivos/itens/ombreiras.txt','r');
		$lista = $file->read_file_armas();
		for($i=0; $i<count($lista); $i++):
			$ombreiras[$i] = explode('-', $lista[$i]);
		endfor;
		$file->close_file();
		
		$escolido = rand (0, count($ombreiras)-1);
		$this->nome = utf8_encode($ombreiras[$escolido][1]);
	}
	...
	function gerar_bonus(){
		...
		switch($nivel):
			case 'fraco':
				for($i=0; $i<count($ombreiras); $i++):
					switch($this->nome):
						case $this->nome ==  utf8_encode($ombreiras[$i][1]):
	...
	function exibir_ombreiras_sorteada($nivel=null){
		$this->init_ombreiras($nivel);
	...
	function init_ombreiras($nivel=null){
		if($nivel != null):
			$this->nivel = rand(1,$nivel);
		else:
			$this->nivel = rand(1,23);
		endif;
		$this->escolher_ombreiras();
...
class LojaDeOmbreiras{
	public $ombreiras;

	function exportar_ombreira_para_txt(){
		$txt_path = "arquivos/exportados/ombreiras/";
		$path = Loader::$path_base.$txt_path;
		$url = Loader::$url_base.$txt_path;
		$arquivo = new Arquivo();
		$data = time('H:i:s').date('dmY');
		$path_final = $path.'Dungeon Evolutio - '.utf8_decode(trim($this->ombreiras[0]->nome).' '.$this->ombreiras[0]->raridade).' ID '.$data.'.txt';
		$url_final = $path.'Dungeon Evolutio - '.utf8_decode($this->ombreiras[0]->nome.' '.$this->ombreiras[0]->raridade).' ID '.$data.'.txt';

		$arquivo->open_file($path_final);
		$arquivo->write_file($this->montar_arquivo_txt_da_ombreira());
	}

	function montar_arquivo_txt_da_ombreira(){
		$_n = "\r\n";
		$arquivo_txt = '';
	
		for($i=0; $i<=count($this->ombreiras)-1;$i++):	
			$arquivo_txt .= 
				'>>>>>>>>>>>>'.trim($this->ombreiras[$i]->nome).' Lv '.$this->ombreiras[$i]->nivel.'<<<<<<<<<<<<'.$_n.
				'Raridade: '.$this->ombreiras[$i]->raridade.$_n.
				'Preço da arma: '.$this->ombreiras[$i]->preco.$_n.
				'Preço total: '.$this->ombreiras[$i]->gerar_preco_total().$_n.
				'Esta arma possui '.$this->ombreiras[$i]->encaixe_livre.' slot de encaixe livre'.$_n.
				'Encaixes '.$this->ombreiras[$i]->encaixes.$_n.
				'Requisito: '.$this->ombreiras[$i]->nivel.'° Nível'.$_n.
				'Nível de poder (NP): '.$this->ombreiras[$i]->np.$_n.
				'RD: '.$this->ombreiras[$i]->rd.$_n.
				'Defesa: '.$this->ombreiras[$i]->defesa.$_n
				;

				
				if($this->ombreiras[$i]->encaixes != 0):
					for($e=1; $e<=($this->ombreiras[$i]->encaixes);$e++):
						if(get_class($this->ombreiras[$i]->pedras[$e]) == 'Pedras_vermelhas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->ombreiras[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->ombreiras[$i]->pedras[$e]->preco.$_n.	
								'Preço para Venda: '.$this->ombreiras[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->ombreiras[$i]->pedras[$e]->nivel.$_n.
								'Bônus de experiência: '.$this->ombreiras[$i]->pedras[$e]->experiencia.$_n.
								'Bônus de dano: '.$this->ombreiras[$i]->pedras[$e]->dano.$_n.
								'Bônus em habilidade (Força): '.$this->ombreiras[$i]->pedras[$e]->habilidade.$_n.
								'Bônus de precisão: '.$this->ombreiras[$i]->pedras[$e]->precisao.$_n;
						
						elseif(get_class($this->ombreiras[$i]->pedras[$e]) == 'Pedras_verdes'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->ombreiras[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->ombreiras[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->ombreiras[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->ombreiras[$i]->pedras[$e]->nivel.$_n.
								'Bônus de esquiva: '.$this->ombreiras[$i]->pedras[$e]->esquiva.$_n.
								'Chance de crítico: '.$this->ombreiras[$i]->pedras[$e]->critico.$_n.
								'Bônus de precisao a distância: '.$this->ombreiras[$i]->pedras[$e]->precisao_distancia.$_n.
								'Bônus em habilidade (Destreza): '.$this->ombreiras[$i]->pedras[$e]->habilidade.$_n;

						elseif(get_class($this->ombreiras[$i]->pedras[$e]) == 'Pedras_roxas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->ombreiras[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->ombreiras[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->ombreiras[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->ombreiras[$i]->pedras[$e]->nivel.$_n.
								'Bônus em habilidade (Vigor): '.$this->ombreiras[$i]->pedras[$e]->vigor.$_n.
								'Bônus de Redução de Dano: '.$this->ombreiras[$i]->pedras[$e]->rd.$_n.
								'Regenera por rodada: '.$this->ombreiras[$i]->pedras[$e]->regeneracao.$_n.
								'Recuperação de vida por acerto: '.$this->ombreiras[$i]->pedras[$e]->vida_por_acerto.$_n;

						elseif(get_class($this->ombreiras[$i]->pedras[$e]) == 'Pedras_amarelas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->ombreiras[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->ombreiras[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->ombreiras[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->ombreiras[$i]->pedras[$e]->nivel.$_n.
								'Bônus de ouro extra a cada sessão: '.$this->ombreiras[$i]->pedras[$e]->ouro_extra.$_n.
								'Bônus em habilidade (Inteligência): '.$this->ombreiras[$i]->pedras[$e]->habilidade.$_n.
								'Aumento de dano em Magias: '.$this->ombreiras[$i]->pedras[$e]->dano.$_n.
								'Aumenta seu DL(Drop Level): '.$this->ombreiras[$i]->pedras[$e]->chance.$_n;

						elseif(get_class($this->ombreiras[$i]->pedras[$e]) == 'Pedras_rosas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->ombreiras[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->ombreiras[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->ombreiras[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->ombreiras[$i]->pedras[$e]->nivel.$_n.
								'Bônus de ouro extra a cada sessão: '.$this->ombreiras[$i]->pedras[$e]->ouro_extra.$_n.
								'Bônus em habilidade (Carisma): '.$this->ombreiras[$i]->pedras[$e]->habilidade.$_n.
								'Aumento de Pvs reservas: '.$this->ombreiras[$i]->pedras[$e]->vida_extra.$_n.
								'Aumenta seu DL(Drop Level)/Def/Esq: '.$this->ombreiras[$i]->pedras[$e]->sorte.$_n;

						elseif(get_class($this->ombreiras[$i]->pedras[$e]) == 'Pedras_azuis'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->ombreiras[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->ombreiras[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->ombreiras[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->ombreiras[$i]->pedras[$e]->nivel.$_n.
								'Bônus de deflexão: '.$this->ombreiras[$i]->pedras[$e]->deflexao.$_n.
								'Bônus em habilidade (Sabedoria): '.$this->ombreiras[$i]->pedras[$e]->habilidade.$_n.
								'Pontos de habilidade (PH): '.$this->ombreiras[$i]->pedras[$e]->pts_hab.$_n.
								'Bônus de dano na habilidade de classe: '.$this->ombreiras[$i]->pedras[$e]->dano.$_n;
						endif;
					endfor;
				endif;
				
				/*	
				if($this->ombreiras[$i]->qtd_habilidade != 0):
					for($h=1; $h<=($this->ombreiras[$i]->qtd_habilidade);$h++):
						$arquivo_txt .= $_n.
							'>>>>>>>>>>>>Habilidades<<<<<<<<<<<<'.$_n.
							'Elemento: '.$this->ombreiras[$i]->habilidades[$h]->elemento.''.$_n.
							'Descrição: '.$this->ombreiras[$i]->habilidades[$h]->descricao;
					endfor;
				endif;
				*/
	...
	function addOmbreiras(Ombreiras $ombreiras){
		$this->ombreiras[] = $ombreiras;		
	}

	function exibir_ombreiras($nivel){
		echo '<div>';
			foreach ($this->ombreiras as $key => $ombreiras):
				$this->ombreiras[$key]->exibir_ombreiras_sorteada($nivel);
			endforeach;
		echo '</div>';	
	}

	function projatar_ombreiras($qtd,$nivel=null){
		$objeto = 'ombreiras_';
		$classe = 'Ombreiras';
		...
		for($i=0; $i<$qtd; $i++):
			$objeto = new $classe();
			$this->addOmbreiras($objeto); 	
		endfor;
		$this->exibir_ombreiras($nivel);
	}
}
