...
class Escudos{
	...
	function escolher_escudo(){
		$file = new Arquivo();
		$escudos = array();
		$file->open_file('arquivos/itens/escudos.txt','r');
		$lista = $file->read_file_armas();
		for($i=0; $i<count($lista); $i++):
			$escudos[$i] = explode('-', $lista[$i]);
		endfor;
		$file->close_file();
		
		$escolido = rand (0, count($escudos)-1);
		$this->nome = utf8_encode($escudos[$escolido][1]);
	}
		...
		switch($nivel):
			case 'fraco':
				for($i=0; $i<count($escudos); $i++):
					switch($this->nome):
						case $this->nome ==  utf8_encode($escudos[$i][1]):
								$this->defesa = (rand(1,3)*$this->nivel)*$this->indice_raridade;
							 	$this->rd = (rand(1,3)*$this->nivel)*$this->indice_raridade;
						break;
					endswitch;
				endfor;
			break;
			...
			case 'medio':
				for($i=0; $i<count($escudos); $i++):
					switch($this->nome):
						case $this->nome ==  utf8_encode($escudos[$i][1]):
								$this->defesa = (rand(4,6)*$this->nivel)*$this->indice_raridade;
							 	$this->rd = (rand(4,6)*$this->nivel)*$this->indice_raridade;
						break;
					endswitch;
				endfor;
			break;
			...
			case 'normal':
				for($i=0; $i<count($escudos); $i++):
					switch($this->nome):
							case $this->nome ==  utf8_encode($escudos[$i][1]):
								 $this->defesa = (rand(7,9)*$this->nivel)*$this->indice_raridade;
								 $this->rd = (rand(7,9)*$this->nivel)*$this->indice_raridade;
						break;
					endswitch;
				endfor;
			break;
			...
			case 'forte':
				for($i=0; $i<count($escudos); $i++):
					switch($this->nome):
							case $this->nome ==  utf8_encode($escudos[$i][1]): 
									$this->defesa = (rand(10,12)*$this->nivel)*$this->indice_raridade;
							   		$this->rd = (rand(10,12)*$this->nivel)*$this->indice_raridade;
						break;
					endswitch;
				endfor;
			break;
			...
			case 'extra_forte':
				for($i=0; $i<count($escudos); $i++):
					switch($this->nome):
						case $this->nome ==  utf8_encode($escudos[$i][1]):
							 	$this->defesa = (rand(13,15)*$this->nivel)*$this->indice_raridade;
							 	$this->rd = (rand(13,15)*$this->nivel)*$this->indice_raridade;
						break;
					endswitch;
				endfor;
			break;
			...
			case 'mega_forte':
				for($i=0; $i<count($escudos); $i++):
					switch($this->nome):
						case $this->nome ==  utf8_encode($escudos[$i][1]):
							 	$this->defesa = (rand(16,18)*$this->nivel)*$this->indice_raridade;
								$this->rd = (rand(16,18)*$this->nivel)*$this->indice_raridade;
						break;
					endswitch;
				endfor;
			break;
			...
			case 'ultra_forte':
				for($i=0; $i<count($escudos); $i++):
					switch($this->nome):
						case $this->nome ==  utf8_encode($escudos[$i][1]):
							 	$this->defesa = (rand(19,21)*$this->nivel)*$this->indice_raridade;
							 	$this->rd = (rand(19,21)*$this->nivel)*$this->indice_raridade;
						break;
					endswitch;
				endfor;
			break;
			...
			case 'Bugado':
				for($i=0; $i<count($escudos); $i++):
					switch($this->nome):
						case $this->nome ==  utf8_encode($escudos[$i][1]):
							 	$this->defesa = (rand(22,30)*$this->nivel)*$this->indice_raridade;
								$this->rd = (rand(22,30)*$this->nivel)*$this->indice_raridade;
						break;
					endswitch;
				endfor;
			break;
	...
	function exibir_escudos_sorteada($nivel=null){
		$this->init_escudos($nivel);
	...
	function init_escudos($nivel=null){
		if($nivel != null):
			$this->nivel = rand(1,$nivel);
		else:
			$this->nivel = rand(1,23);
		endif;
		$this->escolher_escudo();
...
class LojaDeEscudos{
	...
	function exportar_escudo_para_txt(){
		$txt_path = "arquivos/exportados/escudos/";
		$path = Loader::$path_base.$txt_path;
		$url = Loader::$url_base.$txt_path;
		$arquivo = new Arquivo();
		$data = time('H:i:s').date('dmY');
		$path_final = $path.'Dungeon Evolutio - '.utf8_decode(trim($this->escudos[0]->nome).' '.$this->escudos[0]->raridade).' ID '.$data.'.txt';
		$url_final = $path.'Dungeon Evolutio - '.utf8_decode($this->escudos[0]->nome.' '.$this->escudos[0]->raridade).' ID '.$data.'.txt';

		$arquivo->open_file($path_final);
		$arquivo->write_file($this->montar_arquivo_txt_do_escudo());
	}

	function montar_arquivo_txt_do_escudo(){
		$_n = "\r\n";
		$arquivo_txt = '';
	
		for($i=0; $i<=count($this->escudos)-1;$i++):	
			$arquivo_txt .= 
				'>>>>>>>>>>>>'.trim($this->escudos[$i]->nome).' Lv '.$this->escudos[$i]->nivel.'<<<<<<<<<<<<'.$_n.
				'Raridade: '.$this->escudos[$i]->raridade.$_n.
				'Preço da arma: '.$this->escudos[$i]->preco.$_n.
				'Preço total: '.$this->escudos[$i]->gerar_preco_total().$_n.
				'Esta arma possui '.$this->escudos[$i]->encaixe_livre.' slot de encaixe livre'.$_n.
				'Encaixes '.$this->escudos[$i]->encaixes.$_n.
				'Requisito: '.$this->escudos[$i]->nivel.'° Nível'.$_n.
				'Nível de poder (NP): '.$this->escudos[$i]->np.$_n.
				'RD: '.$this->escudos[$i]->rd.$_n.
				'Defesa: '.$this->escudos[$i]->defesa.$_n
				;

				
				if($this->escudos[$i]->encaixes != 0):
					for($e=1; $e<=($this->escudos[$i]->encaixes);$e++):
						if(get_class($this->escudos[$i]->pedras[$e]) == 'Pedras_vermelhas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->escudos[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->escudos[$i]->pedras[$e]->preco.$_n.	
								'Preço para Venda: '.$this->escudos[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->escudos[$i]->pedras[$e]->nivel.$_n.
								'Bônus de experiência: '.$this->escudos[$i]->pedras[$e]->experiencia.$_n.
								'Bônus de dano: '.$this->escudos[$i]->pedras[$e]->dano.$_n.
								'Bônus em habilidade (Força): '.$this->escudos[$i]->pedras[$e]->habilidade.$_n.
								'Bônus de precisão: '.$this->escudos[$i]->pedras[$e]->precisao.$_n;
						
						elseif(get_class($this->escudos[$i]->pedras[$e]) == 'Pedras_verdes'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->escudos[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->escudos[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->escudos[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->escudos[$i]->pedras[$e]->nivel.$_n.
								'Bônus de esquiva: '.$this->escudos[$i]->pedras[$e]->esquiva.$_n.
								'Chance de crítico: '.$this->escudos[$i]->pedras[$e]->critico.$_n.
								'Bônus de precisao a distância: '.$this->escudos[$i]->pedras[$e]->precisao_distancia.$_n.
								'Bônus em habilidade (Destreza): '.$this->escudos[$i]->pedras[$e]->habilidade.$_n;

						elseif(get_class($this->escudos[$i]->pedras[$e]) == 'Pedras_roxas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->escudos[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->escudos[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->escudos[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->escudos[$i]->pedras[$e]->nivel.$_n.
								'Bônus em habilidade (Vigor): '.$this->escudos[$i]->pedras[$e]->vigor.$_n.
								'Bônus de Redução de Dano: '.$this->escudos[$i]->pedras[$e]->rd.$_n.
								'Regenera por rodada: '.$this->escudos[$i]->pedras[$e]->regeneracao.$_n.
								'Recuperação de vida por acerto: '.$this->escudos[$i]->pedras[$e]->vida_por_acerto.$_n;

						elseif(get_class($this->escudos[$i]->pedras[$e]) == 'Pedras_amarelas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->escudos[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->escudos[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->escudos[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->escudos[$i]->pedras[$e]->nivel.$_n.
								'Bônus de ouro extra a cada sessão: '.$this->escudos[$i]->pedras[$e]->ouro_extra.$_n.
								'Bônus em habilidade (Inteligência): '.$this->escudos[$i]->pedras[$e]->habilidade.$_n.
								'Aumento de dano em Magias: '.$this->escudos[$i]->pedras[$e]->dano.$_n.
								'Aumenta seu DL(Drop Level): '.$this->escudos[$i]->pedras[$e]->chance.$_n;

						elseif(get_class($this->escudos[$i]->pedras[$e]) == 'Pedras_rosas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->escudos[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->escudos[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->escudos[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->escudos[$i]->pedras[$e]->nivel.$_n.
								'Bônus de ouro extra a cada sessão: '.$this->escudos[$i]->pedras[$e]->ouro_extra.$_n.
								'Bônus em habilidade (Carisma): '.$this->escudos[$i]->pedras[$e]->habilidade.$_n.
								'Aumento de Pvs reservas: '.$this->escudos[$i]->pedras[$e]->vida_extra.$_n.
								'Aumenta seu DL(Drop Level)/Def/Esq: '.$this->escudos[$i]->pedras[$e]->sorte.$_n;

						elseif(get_class($this->escudos[$i]->pedras[$e]) == 'Pedras_azuis'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->escudos[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->escudos[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->escudos[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->escudos[$i]->pedras[$e]->nivel.$_n.
								'Bônus de deflexão: '.$this->escudos[$i]->pedras[$e]->deflexao.$_n.
								'Bônus em habilidade (Sabedoria): '.$this->escudos[$i]->pedras[$e]->habilidade.$_n.
								'Pontos de habilidade (PH): '.$this->escudos[$i]->pedras[$e]->pts_hab.$_n.
								'Bônus de dano na habilidade de classe: '.$this->escudos[$i]->pedras[$e]->dano.$_n;
						endif;
					endfor;
				endif;
	...
	function addEscudos(Escudos $escudos){
		$this->escudos[] = $escudos;		
	}

	function exibir_escudos($nivel){
		echo '<div>';
			foreach ($this->escudos as $key => $escudos):
				$this->escudos[$key]->exibir_escudos_sorteada($nivel);
			endforeach;
		echo '</div>';	
	}

	function projatar_escudos($qtd,$nivel=null){
		$objeto = 'escudos_';
		$classe = 'Escudos';
		...
		for($i=0; $i<$qtd; $i++):
			$objeto = new $classe();
			$this->addEscudos($objeto); 	
		endfor;
		$this->exibir_escudos($nivel);
	}
}
