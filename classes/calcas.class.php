...
class Calcas{
	...
	function escolher_calcas(){
		$file = new Arquivo();
		$calcas = array();
		$file->open_file('arquivos/itens/calcas.txt','r');
		$lista = $file->read_file_armas();
		for($i=0; $i<count($lista); $i++):
			$calcas[$i] = explode('-', $lista[$i]);
		endfor;
		$file->close_file();
		
		$escolido = rand (0, count($calcas)-1);
		$this->nome = utf8_encode($calcas[$escolido][1]);
	}
	...
	function gerar_bonus(){
		...
		switch($nivel):
			case 'fraco':
				for($i=0; $i<count($calcas); $i++):
					switch($this->nome):
						case $this->nome ==  utf8_encode($calcas[$i][1]):
	...
	function exibir_calcas_sorteada($nivel=null){
		$this->init_calcas($nivel);
	...
	function init_calcas($nivel=null){
		if($nivel != null):
			$this->nivel = rand(1,$nivel);
		else:
			$this->nivel = rand(1,23);
		endif;
		$this->escolher_calcas();
...
class LojaDeCalcas{
	...
	function exportar_calcas_para_txt(){
		$txt_path = "arquivos/exportados/calcas/";
		$path = Loader::$path_base.$txt_path;
		$url = Loader::$url_base.$txt_path;
		$arquivo = new Arquivo();
		$data = time('H:i:s').date('dmY');
		$path_final = $path.'Dungeon Evolutio - '.utf8_decode(trim($this->calcas[0]->nome).' '.$this->calcas[0]->raridade).' ID '.$data.'.txt';
		$url_final = $path.'Dungeon Evolutio - '.utf8_decode($this->calcas[0]->nome.' '.$this->calcas[0]->raridade).' ID '.$data.'.txt';

		$arquivo->open_file($path_final);
		$arquivo->write_file($this->montar_arquivo_txt_da_calca());
	}

	function montar_arquivo_txt_da_calca(){
		$_n = "\r\n";
		$arquivo_txt = '';
	
		for($i=0; $i<=count($this->calcas)-1;$i++):	
			$arquivo_txt .= 
				'>>>>>>>>>>>>'.trim($this->calcas[$i]->nome).' Lv '.$this->calcas[$i]->nivel.'<<<<<<<<<<<<'.$_n.
				'Raridade: '.$this->calcas[$i]->raridade.$_n.
				'Preço da arma: '.$this->calcas[$i]->preco.$_n.
				'Preço total: '.$this->calcas[$i]->gerar_preco_total().$_n.
				'Esta arma possui '.$this->calcas[$i]->encaixe_livre.' slot de encaixe livre'.$_n.
				'Encaixes '.$this->calcas[$i]->encaixes.$_n.
				'Requisito: '.$this->calcas[$i]->nivel.'° Nível'.$_n.
				'Nível de poder (NP): '.$this->calcas[$i]->np.$_n.
				'RD: '.$this->calcas[$i]->rd.$_n.
				'Defesa: '.$this->calcas[$i]->defesa.$_n
				;

				
				if($this->calcas[$i]->encaixes != 0):
					for($e=1; $e<=($this->calcas[$i]->encaixes);$e++):
						if(get_class($this->calcas[$i]->pedras[$e]) == 'Pedras_vermelhas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->calcas[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->calcas[$i]->pedras[$e]->preco.$_n.	
								'Preço para Venda: '.$this->calcas[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->calcas[$i]->pedras[$e]->nivel.$_n.
								'Bônus de experiência: '.$this->calcas[$i]->pedras[$e]->experiencia.$_n.
								'Bônus de dano: '.$this->calcas[$i]->pedras[$e]->dano.$_n.
								'Bônus em habilidade (Força): '.$this->calcas[$i]->pedras[$e]->habilidade.$_n.
								'Bônus de precisão: '.$this->calcas[$i]->pedras[$e]->precisao.$_n;
						
						elseif(get_class($this->calcas[$i]->pedras[$e]) == 'Pedras_verdes'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->calcas[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->calcas[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->calcas[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->calcas[$i]->pedras[$e]->nivel.$_n.
								'Bônus de esquiva: '.$this->calcas[$i]->pedras[$e]->esquiva.$_n.
								'Chance de crítico: '.$this->calcas[$i]->pedras[$e]->critico.$_n.
								'Bônus de precisao a distância: '.$this->calcas[$i]->pedras[$e]->precisao_distancia.$_n.
								'Bônus em habilidade (Destreza): '.$this->calcas[$i]->pedras[$e]->habilidade.$_n;

						elseif(get_class($this->calcas[$i]->pedras[$e]) == 'Pedras_roxas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->calcas[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->calcas[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->calcas[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->calcas[$i]->pedras[$e]->nivel.$_n.
								'Bônus em habilidade (Vigor): '.$this->calcas[$i]->pedras[$e]->vigor.$_n.
								'Bônus de Redução de Dano: '.$this->calcas[$i]->pedras[$e]->rd.$_n.
								'Regenera por rodada: '.$this->calcas[$i]->pedras[$e]->regeneracao.$_n.
								'Recuperação de vida por acerto: '.$this->calcas[$i]->pedras[$e]->vida_por_acerto.$_n;

						elseif(get_class($this->calcas[$i]->pedras[$e]) == 'Pedras_amarelas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->calcas[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->calcas[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->calcas[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->calcas[$i]->pedras[$e]->nivel.$_n.
								'Bônus de ouro extra a cada sessão: '.$this->calcas[$i]->pedras[$e]->ouro_extra.$_n.
								'Bônus em habilidade (Inteligência): '.$this->calcas[$i]->pedras[$e]->habilidade.$_n.
								'Aumento de dano em Magias: '.$this->calcas[$i]->pedras[$e]->dano.$_n.
								'Aumenta seu DL(Drop Level): '.$this->calcas[$i]->pedras[$e]->chance.$_n;

						elseif(get_class($this->calcas[$i]->pedras[$e]) == 'Pedras_rosas'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->calcas[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->calcas[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->calcas[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->calcas[$i]->pedras[$e]->nivel.$_n.
								'Bônus de ouro extra a cada sessão: '.$this->calcas[$i]->pedras[$e]->ouro_extra.$_n.
								'Bônus em habilidade (Carisma): '.$this->calcas[$i]->pedras[$e]->habilidade.$_n.
								'Aumento de Pvs reservas: '.$this->calcas[$i]->pedras[$e]->vida_extra.$_n.
								'Aumenta seu DL(Drop Level)/Def/Esq: '.$this->calcas[$i]->pedras[$e]->sorte.$_n;

						elseif(get_class($this->calcas[$i]->pedras[$e]) == 'Pedras_azuis'):
							$arquivo_txt .= $_n.
								'>>>>>>>>>>>>'.$this->calcas[$i]->pedras[$e]->nome.'<<<<<<<<<<<<'.$_n.
								'Preço de compra: '.$this->calcas[$i]->pedras[$e]->preco.$_n.
								'Preço para Venda: '.$this->calcas[$i]->pedras[$e]->gerar_preco_venda().$_n.
								'Nível da pedra: '.$this->calcas[$i]->pedras[$e]->nivel.$_n.
								'Bônus de deflexão: '.$this->calcas[$i]->pedras[$e]->deflexao.$_n.
								'Bônus em habilidade (Sabedoria): '.$this->calcas[$i]->pedras[$e]->habilidade.$_n.
								'Pontos de habilidade (PH): '.$this->calcas[$i]->pedras[$e]->pts_hab.$_n.
								'Bônus de dano na habilidade de classe: '.$this->calcas[$i]->pedras[$e]->dano.$_n;
						endif;
					endfor;
				endif;
				
				/*	
				if($this->calcas[$i]->qtd_habilidade != 0):
					for($h=1; $h<=($this->calcas[$i]->qtd_habilidade);$h++):
						$arquivo_txt .= $_n.
							'>>>>>>>>>>>>Habilidades<<<<<<<<<<<<'.$_n.
							'Elemento: '.$this->calcas[$i]->habilidades[$h]->elemento.''.$_n.
							'Descrição: '.$this->calcas[$i]->habilidades[$h]->descricao;
					endfor;
				endif;
				*/
	...
	function addCalcas(Calcas $calcas){
		$this->calcas[] = $calcas;		
	}

	function exibir_calcas($nivel){
		echo '<div>';
			foreach ($this->calcas as $key => $calcas):
				$this->calcas[$key]->exibir_calcas_sorteada($nivel);
			endforeach;
		echo '</div>';	
	}

	function projatar_calcas($qtd,$nivel=null){
		$objeto = 'calcas_';
		$classe = 'Calcas';
		...
		for($i=0; $i<$qtd; $i++):
			$objeto = new $classe();
			$this->addCalcas($objeto); 	
		endfor;
		$this->exibir_calcas($nivel);
	}
}
